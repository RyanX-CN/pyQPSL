...
class DAQmxAnalogOutputTask(Structure):
    ...
    def init_task(self, channel_names: List[str], sample_rate: int,
        ...
        if sample_number:
            ...
            self.sample_per_channel = arr2d.shape[1]
    ...
    def unregister_everyn_callback(self) -> int:
        _QPSL_DAQmxAO_register_everyn_callback(pointer(self), 0, c_void_p(),
                                               c_void_p())
        if self.error_code:
            raise BaseException(
                bytes.decode(self.error_buffer, encoding='utf8'))
    ...
    def reset(self) -> int:
        ...
        arr2d = np.zeros((self.channel_number,50))
        arr = arr2d.reshape(arr2d.shape[0] * arr2d.shape[1])
        buffer = (c_double * len(arr))(*arr)
        _QPSL_DAQmxAO_init(pointer(self))
...
try:
    _library = load_dll("QPSL_NIDAQmxAO.dll")
