...
int32 DLL_EXPORT QPSL_DAQmxWriteDigitalLines(DAQmxDigitalOutputTask *task,  int32 *num_of_written, uInt8 *buffer) {
    DAQmxErrChk_task(DAQmxWriteDigitalLines(task->handle, task->sample_per_channel, 0, 10.0, DAQmx_Val_GroupByChannel, buffer, num_of_written, nullptr));
    return 0;
}
...
//     return 0;
// }
int32 DLL_EXPORT QPSL_DAQmxDO_register_everyn_callback(DAQmxDigitalOutputTask *task, uInt32 n_samples, DAQmxEveryNSamplesEventCallbackPtr callback, void *callback_data) {
...
int32 DLL_EXPORT QPSL_DAQmxDO_register_done_callback(DAQmxDigitalOutputTask *task, DAQmxDoneEventCallbackPtr callback, void *callback_data) {
