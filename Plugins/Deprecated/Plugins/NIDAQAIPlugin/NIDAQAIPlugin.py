...
os_path_append("./Plugins/NIDAQAIPlugin/bin")
from Plugins.NIDAQAIPlugin.NIDAQAIAPI import *


class NIDAQAIPluginWorker(QPSLWorker):
    ...
    sig_read_data = pyqtSignal(int, np.ndarray)

    def __init__(self, parent: QWidget, object_name: str, virtual_mode: bool):
        super(NIDAQAIPluginWorker, self).__init__(parent=parent,
    ...
    def on_query_channel(self, device_name: str):
        self.sig_answer_channel_list.emit(
            DAQmxGetAIChannelList(device_name=device_name)[1])

    @QPSLObjectBase.log_decorator()
    def init_task(self, channels: List[Tuple[str, float, float]],
                  sample_rate: float, sample_number: int, everyn: int):
    ...
    def everyn_callback(handle: c_void_p, event_type: c_int32,
        ...
        self: NIDAQAIPluginWorker
        code, arr2d = self.m_task.read_data(read_per_channel=n_sample)
        self.sig_read_data.emit(self.m_read_per_channel, arr2d)
...
class NIDAQAIPluginUI(QPSLTabWidget):
    ...
    sig_init_task, sig_to_init_task = pyqtSignal(list, float, int,
                                                 int), pyqtSignal(
                                                     list, float, int, int)
    ...
    def __init__(self,
                 ...
                 virtual_mode=False,
                 font_family="Arial"):
        super(NIDAQAIPluginUI, self).__init__(parent=parent,
                                              object_name=object_name)
        self.m_font_family = font_family
        self.m_worker = NIDAQAIPluginWorker(self,
                                            object_name="worker",
    ...
    def setupLogic(self):
        ...
        connect_queued(self.m_worker.sig_read_data, self.sig_read_data)
        connect_direct(self.sig_read_data, self.on_data_read)
    ...
    def on_btn_init_task_clicked(self):
        ...
        for channel in self.channels:
            channels.append(
                (channel.cbox_channel.current_text(), channel.spin_min.value(),
                 channel.spin_max.value()))
        ...
        self.sig_to_init_task.emit(channels, sample_rate, sample_number,
                                   everyn)
    ...
    def on_data_read(self, start_index: int, arr2d: np.ndarray):
