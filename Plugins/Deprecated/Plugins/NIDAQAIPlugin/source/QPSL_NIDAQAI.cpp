#ifndef __QPSL_NIDAQAI__
#define __QPSL_NIDAQAI__
#include "NIDAQmx.h"
#include <stdio.h>
#include <stdlib.h>
#ifdef __cplusplus
...
int32 DLL_EXPORT QPSL_DAQmxAI_init(DAQmxAnalogInputTask *task) {
    ...
    for (uInt32 i = 0; i < task->channel_number; i++) {
        DAQmxErrChk_task(DAQmxCreateAIVoltageChan(task->handle, task->channels[i].physical_channel_name, "", DAQmx_Val_Cfg_Default, task->channels[i].min_val, task->channels[i].max_val, DAQmx_Val_Volts, nullptr));
    }
